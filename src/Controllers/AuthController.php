...
final class AuthController extends Controller
{
    ...
    public function login(): void
    {
        ...
        // 🔔 Welcome back in-app notification to the user (students only)
        if ($user['role'] === 'student') {
            Notification::ensureTable();
            Notification::forUser(
                (int) $user['id'],
                '👋 Welcome back, ' . $user['name'] . '!',
                'Good to see you again. Check what\'s new — courses, events and more await.',
                '👋',
                base_url('/student')
            );
        }
    ...
    public function register(): void
    {
        ...
        // 🔔 Welcome notification to the new student (in-app)
        Notification::ensureTable();
        Notification::forUser(
            $userId,
            '👋 Welcome to The Code Munk!',
            'Your account is ready. Explore courses, join events and build your developer portfolio.',
            '🎉',
            base_url('/student')
        );

        // 🔔 Alert admins about new registration (in-app + push)
        Notification::toAdmins(
            '🆕 New Student — ' . Request::string('name'),
            Request::string('name') . ' just signed up with email ' . strtolower(Request::string('email')) . '.',
            '🆕',
            base_url('/admin/students')
        );
        FirebaseNotification::notifyAdmins(
            '🆕 New Student — ' . Request::string('name'),
            Request::string('name') . ' just created an account. View their profile.',
            ['tag' => 'new-student'],
            base_url('/admin/students')
        );
    ...
    public function otpVerify(): void
    {
        ...
        if ($user === null) {
            ...
            // 🔔 Welcome notification (in-app) + admin alert
            Notification::ensureTable();
            Notification::forUser(
                (int) $user['id'],
                '👋 Welcome to The Code Munk!',
            ...
            Notification::toAdmins(
                ...
                $user['name'] . ' just joined via OTP login (' . $email . ').',
                '🆕',
                base_url('/admin/students')
            );
            FirebaseNotification::notifyAdmins(
                '🆕 New Student — ' . $user['name'],
                $user['name'] . ' just created an account via OTP. View their profile.',
                ['tag' => 'new-student'],
                base_url('/admin/students')
            );
        } else {
