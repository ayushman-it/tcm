...
final class EventController extends Controller
{
    ...
    public function store(): void
    {
        ...
        // 🤖 Auto-generate event agenda as live sessions (silent)
        try {
            $agenda = \TCM\Core\AICurriculum::generate(
                Request::string('title'),
                '',   // events don't have duration field
                'beginner',
                Request::string('description', ''),
                'event'
            );
            foreach ($agenda as $i => $session) {
            ...
            error_log('[AI Agenda] Failed for event ' . $id . ': ' . $e->getMessage());
        }

        // 🔔 In-app + push: notify all students about new event
        $type  = Request::string('type', 'free');
        $price = (float) Request::string('price', '0');
        $label = $type === 'paid' ? ' · ₹' . number_format($price) : ' · Free';
        Notification::ensureTable();
        Notification::toStudents(
            '🗓️ New Event — ' . Request::string('title'),
            'A new ' . $type . ' event has been added' . $label . '. Register now!',
            '🗓️',
            base_url('/student/events')
        );
