...
<!-- ── Banner Upload Card ── -->
<div class="tcm-card" style="margin-bottom:16px;padding:0;overflow:hidden;">

    <!-- Banner preview -->
    <div id="bannerPreviewArea" style="position:relative;height:140px;background:#111;overflow:hidden;cursor:pointer;"
        ...
        <?php
        // Check if banner column exists and get value
        $hasBannerCol = isset($profile['banner']);
        $bannerSrc    = $hasBannerCol && !empty($profile['banner'])
                        ? base_url('/uploads/banners/' . basename($profile['banner']))
                        : null;
        ?>

        <?php if ($bannerSrc): ?>
            <img id="bannerImgPreview" src="<?= e($bannerSrc) ?>"
                 style="width:100%;height:100%;object-fit:cover;display:block;" alt="Banner">
        <?php else: ?>
            <div id="bannerPlaceholder" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;">
                <i class="bi bi-image" style="font-size:1.8rem;color:rgba(255,255,255,.25);"></i>
                <span style="font-size:.75rem;font-weight:600;color:rgba(255,255,255,.35);">Click to upload cover banner · JPG, PNG, WEBP · Max 4MB</span>
            </div>
        <?php endif; ?>

        <!-- Change overlay on hover -->
        <div style="position:absolute;inset:0;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity .2s;"
             id="bannerOverlay">
            <div style="text-align:center;">
                <i class="bi bi-camera-fill" style="font-size:1.5rem;color:#fff;display:block;margin-bottom:5px;"></i>
                <span style="font-size:.75rem;font-weight:700;color:#fff;">Change Banner</span>
            </div>
        </div>
    </div>

    <!-- Avatar row below banner -->
    <div style="display:flex;align-items:flex-end;justify-content:space-between;padding:0 20px 14px;margin-top:-28px;flex-wrap:wrap;gap:10px;">
        <!-- Avatar (just display, edit on profile page) -->
        <div style="width:60px;height:60px;border-radius:50%;border:3px solid #fff;
                    background:#f0f0f0;overflow:hidden;flex-shrink:0;
                    display:grid;place-items:center;font-size:1.3rem;font-weight:800;color:#111;
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;padding-bottom:4px;">
            ...
            <form method="post" action="<?= base_url('/student/portfolio/banner') ?>"
                  enctype="multipart/form-data" id="bannerForm">
                <?= csrf_field() ?>
                <input type="file" id="bannerFileInput" name="banner"
                       accept="image/jpeg,image/png,image/webp"
                       style="display:none;"
                       onchange="document.getElementById('bannerForm').submit()">
                <button type="button" class="tcm-btn sm" onclick="document.getElementById('bannerFileInput').click()">
                    <i class="bi bi-image"></i> <?= $bannerSrc ? 'Change Banner' : 'Add Banner' ?>
                </button>
            </form>

            <?php if ($bannerSrc): ?>
            <!-- Remove banner -->
            <form method="post" action="<?= base_url('/student/portfolio/banner/remove') ?>"
                  onsubmit="return confirm('Remove banner?');">
                <?= csrf_field() ?>
                <button class="tcm-btn sm danger"><i class="bi bi-trash"></i> Remove</button>
            </form>
            <?php endif; ?>
        </div>
    </div>
...
var bannerArea = document.getElementById('bannerPreviewArea');
var bannerOverlay = document.getElementById('bannerOverlay');
if (bannerArea && bannerOverlay) {
    bannerArea.addEventListener('mouseenter', function(){ bannerOverlay.style.opacity='1'; });
    bannerArea.addEventListener('mouseleave', function(){ bannerOverlay.style.opacity='0'; });
}
</script>
